...
// Difficulty config with colors, names, and descriptions
const DIFFICULTY_CONFIG = [
	{
		name: "EASY",
		color: Color3.fromRGB(0, 255, 100),
		description: "Relaxed experience.\nEnemies deal less damage.\nGreat for learning the game.",
	},
	{
		name: "NORMAL",
	...
	},
	{
		...
		description: "Punishing difficulty.\nRequires skilled play and coordination.\nSignificantly better rewards.",
	},
	{
		name: "NIGHTMARE",
...
export class DifficultyPedestalComponent extends StationComponent {
	...
	onStart() {
		super.onStart();
		print("[Lobby] DifficultyPedestal component started");
		...
		this.lobbyController.subscribe((state) => {
			...
			print(`[Lobby] DifficultyPedestal - setting difficulty to: ${difficulty}`);
	...
	private createDescriptionBillboard() {
		...
		billboardGui.Name = "DifficultyDescriptionGui";
	...
	protected onTriggered() {
		print("[Lobby] DifficultyPedestal triggered");
		const state = this.lobbyController.getState();
		if (!state.room) {
			print("[Lobby] DifficultyPedestal - no room, ignoring");
			return;
		}
		...
		print(`[Lobby] DifficultyPedestal - cycling to difficulty: ${nextDifficulty}`);
